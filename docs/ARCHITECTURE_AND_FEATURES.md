...
一站式学语言助手基于著名的 **"i+1"** 理论（可理解输入理论），通过智能地将网页内容中的部分词汇替换为目标语言，创造一个沉浸式的语言学习环境。用户在日常浏览网页的过程中，自然地接触和学习新的词汇，实现"润物细无声"的语言学习体验。
...
- [x] **单词收藏和复习功能**（云端词汇管理）
- [x] **学习进度追踪**（连续学习天数、活动记录）
- [x] **成就系统**（用户等级、成就徽章）
- [x] **每日目标系统**（目标设置、进度追踪）
- [x] **自定义词汇列表**（词汇分类管理）
- [x] **数据导出功能**（JSON/CSV/Anki格式）
