...
import { WordPopupService } from '@/src/modules/wordPopup';
...
export class ContentManager implements IContentManager {
  ...
  destroy(): void {
    try {
      ...
      // 销毁单词弹窗服务
      WordPopupService.getInstance().destroy();
   ...
   */
  private initializeWordPopup(): void {
    const wordPopupService = WordPopupService.getInstance();
    wordPopupService.initialize();
    
    // Set up event handler for popup events (favorite, audio, etc.)
    wordPopupService.setEventHandler((type, data) => {
      console.log(`[ContentManager] Word popup event: ${type}`, data);
      // TODO: Handle events when VocabularyService is implemented
    });
  }
